...
function HeroWithSearch(props) {
  const { choosedLink, searchValue, chooseRegion, closeListRegion, openList, openSearchRegion, breadcrumbsTitle } = props;
      ...
      <Grid container sx={{ display: 'flex', flexDirection: 'column' }}>
        <Grid item sx={{ zIndex: '2', marginTop: mobileSize ? '70px' : '-20px' }}>
          ...
          <Typography variant='p' textTransform={'capitalize'} color="white" style={{ zIndex: 1, fontSize: '20px', display: openSearchRegion ? 'flex' : 'none', alignItems: 'center' }} onClick={closeListRegion}>
            {breadcrumbsTitle} in <Typography id="region-text" variant='p' color="white" style={{ zIndex: 1, fontSize: '20px', fontWeight: 'bold', marginLeft: '10px', cursor: 'pointer' }}>Jakarta Selatan</Typography><KeyboardArrowDown sx={{ cursor: 'pointer' }} id="arrow-region"/>
          </Typography>
          <List sx={{ backgroundColor: 'white', position: 'absolute', left: mobileSize ? '130px' : websiteMediumSize ? '260px' : '560px', top: mobileSize ? '185px' : websiteMediumSize ? '160px' : '190px', padding: '0px', width: '270px', boxShadow: '1', display: openList ? 'block' : 'none' }}>
            ...
            <ListItem disablePadding>
              <ListItemButton onClick={chooseRegion}>
                <ListItemText primary="Jakarta Pusat"/>
              </ListItemButton>
            </ListItem>
            <Divider light/>
            <ListItem disablePadding>
              <ListItemButton onClick={chooseRegion}>
                <ListItemText primary="Jakarta Barat"/>
              </ListItemButton>
            </ListItem>
            <Divider light/>
            <ListItem disablePadding>
              <ListItemButton onClick={chooseRegion}>
                <ListItemText primary="Jakarta Timur"/>
              </ListItemButton>
            </ListItem>
            <Divider light/>
            <ListItem disablePadding>
              <ListItemButton onClick={chooseRegion}>
                <ListItemText primary="Jakarta Selatan"/>
              </ListItemButton>
            </ListItem>
            <Divider light/>
            <ListItem disablePadding>
              <ListItemButton onClick={chooseRegion}>
                <ListItemText primary="Jakarta Utara"/>
              </ListItemButton>
            </ListItem>
            <Divider light/>
          </List>
...
function HeroWithoutSearch(props) {
  ...
  return (
    <div style={{ marginTop: mobileSize ? '90px' : '10px' }}>
      ...
      <Grid container sx={{ marginTop: mobileSize ? '80px' : '50px' }}>
...
export const Hero = (props) => {
  ...
  const [searchValue, setSearchValue] = useState(["Jakarta JWT, 2000", "Jakarta JWT, 2001", "Jakarta JWT, 2002", "Bandung BND, 2023"])
  const choosedLink = (e) => {
    let color = ""
    if (e === path) {
  ...
  const { banner, withSearch, chooseRegion, closeListRegion, openList, openSearchRegion, breadcrumbsTitle } = props;
  let component
  if (withSearch) {
    component = <HeroWithSearch handleChangeSubMenu={props.handleChangeSubMenu} openList={openList} breadcrumbsTitle={breadcrumbsTitle} openSearchRegion={openSearchRegion} closeListRegion={closeListRegion} chooseRegion={chooseRegion} choosedLink={choosedLink} searchValue={searchValue} searching={props.searching} />
  } else {
