...
export default function InterestRates() {
  ...
  const dataInterestRates = async (needRefreshPage) => {
    try {
      const response = await axios.get(`${config.baseURL}/article?offset=0&limit=10&articleHeader=1&articleSubHeader=1`);
  ...
  return (
    <div>
      ...
      <div className='box-content' style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div className="p-0" style={{ marginTop: '60px', }}>
          <h3>Mortgage and Finance</h3>
          <div className="row">
            {data.map(item => (
              <div key={item.articleId} className="col-lg-4 col-md-12 mb-5">
                <img src={item.image2} style={{ height: '240px', width: '100%', }} />
                <div style={{ padding: "0px 0px 0px" }}>
                  <h1 className="mt-4 text-center"
                    style={{
                      color: '#6E6E6E',
